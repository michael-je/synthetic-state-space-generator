...
def BFS(root: Node, max_depth: int = 10): #tekur ca 5 mín að runna max_depth = 10
...
print("###############################################")
print("average branching factor by depth")

print(f"{'Depth':>5} | {'TermRatio':>10} | {'Terminal':>9} | {'Unique':>9} | {'AvgBF':>6}")
print("-" * 50)

for depth in range(start_depth, max_depth + 1):
    total_nodes = len(depth_sorted_nodes[depth])
    ...
    if total_nodes == 0:
        ...
        unique = total_nodes
        try:
            ratio = terminals / sum(stats[1:])
        except ZeroDivisionError:
            ratio = 1

        total_children = sum(len(node.children) for node in depth_sorted_nodes[depth])
        avg_bf = total_children / total_nodes

    print(f"{depth:>5} |     {ratio:>6.3f}      |   {terminals:>7}  |     {unique:>6}     |  {avg_bf:>6.3f}")

# print("\nNon-unique average branching factors (weighted by transpositions):")
# for depth in range(start_depth, max_depth + 1):
#     print(f"Depth {depth}: {avg_bf_nonunique[depth]:.3f}")
...
# Terminal ratios (0 branching factor / sum of all others)
print("\nunique:")
for depth in range(start_depth, max_depth + 1):
    stats = bf_by_depth_unique[depth]
    total_states = sum(stats)
    try:
        ratio = stats[0] / sum(stats[1:])
    except ZeroDivisionError:
        ratio = 1
    print(f"Depth {depth}: Terminal ratio: {ratio:.3f}, Terminal count: {stats[0]}, Unique states: {total_states}")
...
for depth in range(start_depth, max_depth + 1):
    ...
    try:
        ...
        ratio = 1
    print(f"Depth {depth}: Terminal ratio: {ratio:.3f}, Terminal count (weighted): {stats[0]}")
# # Unique
# total = 0
# terminals_cnt = 0
# for depth in range(start_depth, max_depth + 1):
